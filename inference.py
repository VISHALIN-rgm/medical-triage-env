...
# Balanced parameters for 85-92% accuracy
EPSILON = 0.10
EPSILON_DECAY = 0.995
EPSILON_MIN = 0.05
ALPHA = 0.11
...
# INVESTIGATE threshold (80% confidence)
INVESTIGATE_CONFIDENCE_THRESHOLD = 0.80

# Realistic error rate (produces 85-92% accuracy)
ERROR_RATE = 0.10
...
# Real patient names from MIMIC dataset
REAL_PATIENT_NAMES = [
...
if HF_TOKEN is None:
    print("⚠️ WARNING: HF_TOKEN not set. LLM will not work.", file=sys.stderr)
...
class PersistentQLearning:
    def __init__(self, epsilon=EPSILON):
        ...
        self._load()
        print(f"🧠 Q-Learning | ε={self.epsilon:.2f}", file=sys.stderr)
    ...
    def select_action(self, news_score, guideline_action, confidence):
        state = self._news_to_bucket(news_score)
    ...
    def get_q_value(self, news_score, action):
        state = self._news_to_bucket(news_score)
        return self.q_table[state].get(action, 0.0)

    def update(self, news_score, action, reward):
        state = self._news_to_bucket(news_score)
...
class RealClinicalAgent:
    ...
    def make_decision(self, patient):
        ...
        if requires_investigation and not self.investigate_triggered:
            ...
            action = self.rl.select_action(news_score, guideline_action, confidence)
        ...
        q_value = self.rl.get_q_value(news_score, action)
...
# ============== TASK RUNNER (HACKATHON COMPLIANT OUTPUT) ==============
...
def run_task(task_id: str, agent: RealClinicalAgent, data_loader: RealDataLoader) -> Dict:
    ...
    # REQUIRED HACKATHON FORMAT: [START] line
    ...
    for patient in obs.patients:
        ...
        # REQUIRED HACKATHON FORMAT: [STEP] line
    ...
    # REQUIRED HACKATHON FORMAT: [END] line with score field
    print(f"[END] success=true steps={step} score={normalized_score:.3f} rewards={rewards_str}")

    return {"task": task_id, "score": capped_score, "max_score": max_score, "steps": step, "normalized_score": normalized_score}
