"""
Medical Triage Environment - Core Environment Logic
"""
...
class MedicalTriageEnvironment:
    ...
    def _handle_escalation(self, action):
    ...
    def _handle_discharge(self, action):
        patient = self._find_patient(action.patient_id)
        if not patient:
            return -0.1
    ...
    def _handle_test(self, action):
        self.resource_usage["tests"] += 1
        if self.resource_usage["tests"] > 5:
            return -0.1
        return 0.1
    
    def _handle_treatment(self, action):
    ...
    def _get_observation(self):
        return MedicalObservation(
            patients=copy.deepcopy(self.patients),
            available_tests=["CBC", "ECG", "CXR", "Troponin"],
    ...
    def _get_symptoms(self, urgency):
        if urgency > 0.7:
            return ["chest pain", "shortness of breath", "dizziness"]
        elif urgency > 0.3:
            return ["fever", "cough", "fatigue"]
        else:
            return ["headache", "runny nose", "mild pain"]
    
    def _get_vitals(self, urgency):
        if urgency > 0.7:
            return VitalSigns(
    ...
    def _get_chief_complaint(self, urgency):
        if urgency > 0.7:
